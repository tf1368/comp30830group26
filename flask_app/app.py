...
@app.route('/prediction/<int:station_number>')
def predict(station_number):
    ...
    prediction_list = []
    for i in [3,2,1,0]:
        availability_prediction = rfc1.predict([[station_number,df1["bike_stands"],df1["position_lat"],df1["position_long"],0.965926,0.258819,df2.loc[i:i,"Avg_temp"],df2.loc[i:i,"Avg_pressure"],df2.loc[i:i,"Avg_humidity"],df2.loc[i:i,"Avg_visibility"],df2.loc[i:i,"Avg_wind_speed"],df2.loc[i:i,"Avg_wind_degree"],df2.loc[i:i,"Avg_clouds_all"],True]])
